// BUILT THE FUNCTIONALITY FOR A "CHOOSE YOUR OWN ADVENTURE" GAME
...
function renderCharacter(scene, key) {
  if (gameState.character) {
    ...
    gameState.character = scene.add.image(270, 340, key).setOrigin(.5, 1).setScale(.7)
    // gameState.character = scene.add.image(270, 340, key)
    // gameState.character.setOrigin(.5, 1).setScale(.7)
...
function displayPage(scene, page) {
  ...
  for (let i=0; i<page.options.length; i++) {
    ...
    // destroys current page and displays the next page
    ...
    // highlights textbox when hovered over
    ...
    // gets rid of highlight when moving off box
...
function fetchPage(page) {
   ...
   const pages = [
...

    {
      character: 'orc',
      page: 29,
      narrative: 'Orc: Why would I befriend you?  Get fucked!\nTHE END',
      options: [
