...
//   The method Phaser.Utils.Array.GetRandom() takes an array and selects a random element in 
//   that array. .getChildren() is called on a Group object and returns an array of the sprites 
...
function preload() {
  ...
  function update() {
      ...
      if (numOfTotalEnemies() === 0) {
        ...
        gameState.enemyVelocity = 1
        this.physics.pause()
        this.add.text(150, 250, 'YOU WIN!!! \n Click to restart', { fontSize: '15px', fill: '#000000' })
      } else {
        gameState.enemies.getChildren().forEach(bug => bug.x += gameState.enemyVelocity)
